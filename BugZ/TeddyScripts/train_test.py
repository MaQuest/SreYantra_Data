...
def local_train_test(dependent_pairs, independent_pairs, project, binaryclass, multiclass, train_size,test_size):
    ...
    if (binaryclass):
        ...
        requires_sub = dp_subset[dp_subset["MultiClass"] == 1]
        blocks_sub = dp_subset[dp_subset["MultiClass"] == 2]
        
        test_df = requires_sub.sample(int(test_size/3))
        test_df = test_df.append(blocks_sub.sample(int(test_size/3)), ignore_index = True)
        test_df = test_df.append(ip_subset.sample(int(test_size/3)), ignore_index = True)                      
...
def global_d_sets(dp, ip, train_project, test_project, binaryclass, multiclass, train_size, test_size, cp_size):
    ...
    train_df = dp[(dp["req1Product"] == train_project) & (dp["req2Product"] == train_project)].sample((train_size - cp_size)/2))
    ...
    train_df = train_df.append(ip[(ip["req1Product"] == train_project) & (ip["req2Product"] == train_project)].sample((train_size - cp_size)/2))
    train_df = train_df.sample(frac = 1)
    
    # obtain test set
    test_df = dp[(dp["req1Product"] == test_project) & (dp["req2Product"] == test_project)].sample(test_size/2))
    test_df = test_df.append(ip[(ip["req1Product"] == test_project) & (ip["req2Product"] == test_project)].sample(test_size/2))
    test_df = test_df.sample(frac = 1)
