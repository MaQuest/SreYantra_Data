...
These have non empty depends_on and Blocks fields. 

Idea: is to iterate through each one of these ID's and then download the data for each item again using 
REST API call : https://bugzilla.mozilla.org/rest/bug/1572368
...
INPUT = "./TasksAndEnhancements.json"
STAGE1_OUTPUT = "AllData.csv"

STAGE2_INPUT = "AllData.csv"
STAGE2_OUTPUT = "AllData.csv"
...
def FetItemInfo(id):
    ...
    #print (type(ele['bugs'][0]['depends_on']),type(ele['bugs'][0]['depends_on'][0]), ele['bugs'][0]['depends_on'][0])
    #print (ele['bugs'][0])
    #print (ele['bugs'][0]['depends_on'])
    #print ("-------------------------------------------------------------------------------------\n")
...
def stage1():
    ...
    #df_AllData.to_csv("dummy.csv")
    #print(df_data.head())    


import itertools #to join list of lists
...
def main():
    #stage1() # done just once as it uses seed list to harness the data
    #stage2(STAGE2_INPUT,"ListAllStage2IDS.txt")
    stage3()
